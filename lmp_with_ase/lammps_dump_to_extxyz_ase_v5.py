...
추가:
- log.lammps에서 timestep별 energy(PotEng 또는 TotEng) 읽어서 extxyz info에 저장
...
OUT_EXTXYZ = "traj_H2O_551.extxyz"
...
TYPEMAP_TXT = None
DATA_FILE   = "../input_struct/551_H2O.data"
...
# log.lammps에서 energy 읽기
WRITE_ENERGY = True
LOG_LAMMPS   = "log.lammps"
ENERGY_KEY   = "PotEng"   # "PotEng" or "TotEng"
...
def read_log_energies(logfile, energy_key="PotEng"):
    p = Path(logfile)
    if not p.exists():
        raise FileNotFoundError(f"log.lammps not found: {logfile}")
    ...
    while i < len(lines):
        ...
        # thermo header line, e.g.:
        # Step Temp PotEng TotEng Press ...
        if "Step" in cols and energy_key in cols:
            step_col = cols.index("Step")
            eng_col = cols.index(energy_key)
            ...
            while j < len(lines):
                ...
                if not parts:
                    break
                if len(parts) <= max(step_col, eng_col):
                    break

                try:
                    step = int(float(parts[step_col]))
                    eng = float(parts[eng_col])
                    energy_map[step] = eng
                except ValueError:
                    break
    ...
    if not energy_map:
        raise RuntimeError(
            f"log.lammps에서 '{energy_key}'를 찾지 못했습니다. "
            f"thermo_style에 {energy_key}가 포함되어 있는지 확인하세요."
        )
...
def main():
    ...
    if WRITE_ENERGY:
        energy_map = read_log_energies(LOG_LAMMPS, ENERGY_KEY)
        print(f"[INFO] loaded energies from {LOG_LAMMPS}: {len(energy_map)} steps")
    else:
        energy_map = {}
    ...
    while i < len(lines):
        ...
        if WRITE_ENERGY:
            if step in energy_map:
                at.info["energy"] = float(energy_map[step])
            else:
                n_missing_energy += 1
                print(f"[WARN] no {ENERGY_KEY} found in log.lammps for step={step}")
    ...
    if WRITE_ENERGY:
        print(f"[INFO] energy key used: {ENERGY_KEY}")
        print(f"[INFO] frames missing energy: {n_missing_energy}")
