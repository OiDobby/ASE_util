...
dist_from_slab = 2.0  # Minimum distance from the slab
dist_from_max = 3.0  # Minimum distance from the electrolyte
# ======================
...
all_positions = np.array([atom.position for atom in structure])  # Include all existing atom positions

# Separate slab atoms and H2O molecules
...
if len(h2o_positions) == 0:
    raise ValueError("H2O molecules not found.")

# Find the highest z-coordinate of the slab and H2O molecules
slab_max_z = slab_positions[:, 2].max()
h2o_max_z = h2o_positions[:, 2].max()

# Define the z-coordinate range for inserted atoms
z_min = slab_max_z + dist_from_slab  # At least 2 Å above the slab top
z_max = h2o_max_z - dist_from_max  # Below the top of the H2O region

# Define the x, y coordinate range
...
while len(added_atoms) < num_atoms and iterations < max_iterations:
    ...
    # Wrap the candidate position into the PBC cell
    candidate_wrapped = wrap_positions(candidate[np.newaxis, :], cell=cell)[0]

    # Calculate distances to all existing atoms under PBC
    distances = []
    ...
    # Accept the candidate if the minimum-distance criterion is satisfied
